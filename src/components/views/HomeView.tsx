...
export const HomeView = ({ news, onNewsClick, onNavigate }: HomeViewProps) => {
    ...
    useEffect(() => {
        ...
        // Force refresh of the local image
        const timestamp = new Date().getTime();
        setBannerUrl(`/assets/news/hero/${regionKey}/latest.png?t=${timestamp}`);

        // Originally tried Firebase first, but for local dev/demo we prioritize the generated file
        /*
        import('../../firebaseConfig').then(({ storage }) => {
            import('firebase/storage').then(({ ref, getDownloadURL }) => {
                const bannerRef = ref(storage, `banners/${regionKey}/latest.png`);
                getDownloadURL(bannerRef)
                    .then((url) => setBannerUrl(url))
                    .catch((err) => {
                        console.log("Cloud banner not found", err);
                        setBannerUrl(`/assets/news/hero/${regionKey}/latest.png?t=${timestamp}`);
                    });
            });
        });
        ...
        <div className="home-container" style={{ padding: '0 4px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                ...
                {featuredNews && (
                    ...
                    >
                        <img
                            src={bannerUrl || featuredNews.image_url || featuredNews.image}
                            alt="News"
                            style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                            onError={(e) => {
                                // Fallback to news image if banner missing
                                if (featuredNews.image_url) e.currentTarget.src = featuredNews.image_url;
                            }}
                        />
                        <div style={{
