...
export const HomeView = ({ news, onNewsClick, onNavigate }: HomeViewProps) => {
    ...
    useEffect(() => {
        ...
        // Try to load from Firebase Storage (autogenerated by Cloud Function)
        // We import these dynamically or assume they are available if initialized
        import('../../firebaseConfig').then(({ storage }) => {
            import('firebase/storage').then(({ ref, getDownloadURL }) => {
                ...
                getDownloadURL(bannerRef)
                    .then((url) => {
                        console.log("Loaded banner from cloud:", url);
                        setBannerUrl(url);
                    })
                    .catch((err) => {
                        console.log("Cloud banner not found, using default.", err);
                        // Fallback to local
